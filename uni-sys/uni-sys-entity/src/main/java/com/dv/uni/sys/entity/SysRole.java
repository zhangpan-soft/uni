...
public class SysRole implements BaseEntity<String> {
    @Transient
    ...
    @Column(length = 1, columnDefinition = "VARCHAR(1) COMMENT '角色类型'")
    @ApiModelProperty("角色类型")
    @Length(max = 1)
    private String type;

    @ManyToMany(cascade = {CascadeType.ALL})
    @JoinTable(name = "sys_role_resource_mapping", joinColumns = {@JoinColumn(name = "role_id")}, inverseJoinColumns = {@JoinColumn(name = "resource_id")})
