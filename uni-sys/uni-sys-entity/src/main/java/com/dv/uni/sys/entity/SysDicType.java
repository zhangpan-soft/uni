...
public class SysDicType implements BaseEntity<String> {
    @Transient
    ...
    @Column(columnDefinition = "VARCHAR(1) COMMENT '是否系统字典'")
    @Length(max = 1)
    ...
    @OneToMany(mappedBy = "dicDataId")
    @ApiModelProperty("字典数据")
    private List<SysDicData> dataList;
}
